...
\begin{enumerate}[leftmargin=*]
  ...
  \end{itemize}
  \item Ensure ports 3000, 4000, 7077, 8001, 8080, 8081, 8082, 8086, 8800, 35000, 350001 are free.
\end{enumerate}
...
\begin{itemize}[leftmargin=*]
  ...
  \begin{itemize}[leftmargin=*]
    ...
    \item List services: \texttt{sudo docker stack services distributed} (or \texttt{sudo docker service ls})
...
\begin{itemize}[leftmargin=*]
  ...
  \begin{itemize}[leftmargin=*]
    ...
    \item \texttt{DELETE /trades/\{symbol\}} --- delete/sell a trade; optional body \texttt{\{"sale\_price":190\}} to compute net gain.
    \item \texttt{GET /cache} --- view in-memory price cache (refreshed every 5s for tracked symbols).
    \item Price polling (optional): backend can poll quotes for symbols in \texttt{PRICE\_SYMBOLS} every \texttt{PRICE\_POLL\_INTERVAL} seconds and write to Influx (\texttt{prices} measurement) when \texttt{INFLUXDB\_TOKEN} is set.
  \end{itemize}
